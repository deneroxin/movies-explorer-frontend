...
export default function SignPane({
  ...
  return (
    <div className={`sign-pane ${className}`}>
      <Link className="sign-pane__logo site-logo interactive_type_2" to="/" />
      ...
      <form name="sign-pane" noValidate={true} className="sign-pane__form" onSubmit={handleSubmit}>
        ...
        <button type="submit"
          className={`sign-pane__submit interactive_type_2${children.length < 3 ? ' extra-space' : ''}`}
      ...
      <nav className="sign-pane__nav">
        ...
        <Link className="sign-pane__link interactive_type_2" to={linkPath}>
          {linkText}
        </Link>
      </nav>
    </div>
